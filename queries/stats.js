...
const getStatsByUserId = async (user_id) => {
    try {
        const statsByUserId  = await db.any(`SELECT * FROM stats WHERE user_id = $1`, user_id);
...
const updateUserStats = async (stats) => {
  try {
    const updatedUserStats = await db.one(
      `UPDATE stats SET xp=$1, games_played=$2, questions_correct=$3, questions_wrong=$4 WHERE user_id=$5 AND id=$6 RETURNING *`,
      [
        stats.xp,
        stats.games_played,
